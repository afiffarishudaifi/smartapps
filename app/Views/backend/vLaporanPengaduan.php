    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#select_kategori').select2({
                ...
                theme: 'bootstrap4',
                ajax: {
                    url: '<?php echo base_url('Dashboard/M_laporan/data_kategori'); ?>',
                    dataType: 'json'
                }
            });
            /* Isi Table */
            $('.table').DataTable({
                ...
                "ajax": {
                    "url": "<?= base_url() ?>/Dashboard/M_laporan/data/" + $('#tanggal').val() + '/' + $('#select_kategori').val() + '/' + $('#select_status').val(),
        ...
        function ganti(kategori, status) {
            $('.table').DataTable().ajax.url('<?= base_url() ?>/Dashboard/M_laporan/data/' + $('#tanggal').val() + '/' + kategori + '/' + status).load();
        };

        $('#tanggal').on('apply.daterangepicker', function(ev, picker) {
            var tanggal = picker.startDate.format('DD-MM-YYYY') + ' - ' + picker.endDate.format('DD-MM-YYYY');
            $('.table').DataTable().ajax.url('<?= base_url() ?>/Dashboard/M_laporan/data/' + tanggal + '/' + $('#select_kategori').val() + '/' + $('#select_status').val()).load();
        });
