                        ...
                        <div class="panel-body">
                            <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select id="select_status" name="status" class="form-control" style="width: 100%;" onchange="ganti($('#select_kategori').val(), this.value)">
                                        <option value="null">Pilih Status Tempat</option>
                            ...
                            <table id="data-table-combine table" style="width: 100%" class="table table-stripe table-responsive table-bordered table-td-valign-middle" width="100%">
                                ...
                                <thead>
                                    <tr>
                                        <th width="1%">ID</th>
                                        <th class="text-nowrap">Nama Tempat</th>
                                        <th class="text-nowrap">Deskripsi Tempat</th>
                                        <th class="text-nowrap">Alamat Tempat</th>
                                        <th class="text-nowrap">No Telp</th>
                                        <th class="text-nowrap">Status Tempat</th>
                                    </tr>
                                </thead>
                            </table>
                        </div>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#select_kategori').select2({
                ...
                theme: 'bootstrap4',
                ajax: {
                    url: '<?php echo base_url('Dashboard/M_laporan/data_kategori_tempat'); ?>',
                    dataType: 'json'
                }
            });
            /* Isi Table */
            $('.table').DataTable({
                ...
                "ajax": {
                    "url": "<?= base_url() ?>/Dashboard/M_laporan/data_tempat/" + $('#tanggal').val() + '/' + $('#select_kategori').val() + '/' + $('#select_status').val(),
                    "dataSrc": ""
                },
                "columns": [{
                        "data": "ID_TEMPAT"
        ...
        function ganti(kategori, status) {
            $('.table').DataTable().ajax.url('<?= base_url() ?>/Dashboard/M_laporan/data_tempat/' + $('#tanggal').val() + '/' + kategori + '/' + status).load();
        };

        $('#tanggal').on('apply.daterangepicker', function(ev, picker) {
            var tanggal = picker.startDate.format('DD-MM-YYYY') + ' - ' + picker.endDate.format('DD-MM-YYYY');
            $('.table').DataTable().ajax.url('<?= base_url() ?>/Dashboard/M_laporan/data_tempat/' + tanggal + '/' + $('#select_kategori').val() + '/' + $('#select_status').val()).load();
        });
